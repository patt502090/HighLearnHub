...
function calculatePercentage(watchTime, totalDuration) {
  ...
  return Math.round((totalDuration / watchTime) * 100);
}
                  ...
                  <Link to={`/mycourse/${course.id}`} key={course.id} title="ดูคลิปวิดิโอ">
                    <div className="p-4 bg-gray-100 rounded-lg shadow-md hover:translate-y-[-2px] transition-transform duration-300 w-full">
                      ...
                      <div className="mt-auto">
                        <div className="text-sm font-base text-yellow-700 mb-1">เรียนไปแล้วทั้งหมด {course.percentage} %</div>

                        <Progress progress={course.percentage} color="yellow" size="sm" />
                      </div>
                    </div>
                  </Link>
                ))}
